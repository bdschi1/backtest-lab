...
def _moments_mk(k: int, rho: float = 0.0):
    ...
    sqrt_rho = math.sqrt(rho) if rho > 0 else 0.0
    ...
    def cdf_mk(z):
        # P(M_k <= z) = E[ Φ((z - sqrt(rho)*W) / sqrt(1-rho))^k ]
        # where W ~ N(0,1), computed via outer expectation
        inner = norm.cdf((z - sqrt_rho * 0.0) / sqrt_1mrho) ** k
        return inner

    # E[M_k] = integral z * f_Mk(z) dz = integral (1 - F_Mk(z)) dz for z>0
    #          minus integral F_Mk(z) dz for z<0
    # Use: E[g(M_k)] via the identity with the CDF of the max
    # More directly: E[M_k] via Gauss-Hermite on the outer expectation

    def _e_mk_integrand(w):
        """E[M_k] using the identity E[M_k] = E_W[ E[ max_i (sqrt(rho)*W + sqrt(1-rho)*X_i) ] ]
        where X_i are iid N(0,1).
        = E_W[ sqrt(rho)*W + sqrt(1-rho) * E[max(X_1,...,X_k)] ]  (by linearity + iid)
        This simplification doesn't work because the max is inside.
        Use the CDF approach instead.
        """
        # For each value of the common factor W=w,
        # the conditional variables are iid N(sqrt_rho*w, 1-rho)
        # max of k iid N(mu, sigma²) has E[max] that we compute via GH
        mu = sqrt_rho * w
        # E[max of k iid N(mu, (1-rho))] = mu + sqrt(1-rho) * E[max of k N(0,1)]
        # So we just need E[max of k standard normals] once
        pass

    # Simpler: compute E[max of k standard normals] by GH quadrature
    # Then scale by correlation structure
    # E[max(Z_1,...,Z_k)] for iid standard normals
    def _phi_k(z):
        return norm.cdf(z) ** k

    def _pdf_max_iid(z):
        return k * norm.pdf(z) * norm.cdf(z) ** (k - 1)

    e_max_iid = E(lambda z: z * _pdf_max_iid(z) / norm.pdf(z) * norm.pdf(z))
    # Simplify: E[z * k * phi(z) * Phi(z)^{k-1}] but with GH weights already
    # include the pdf. Let's compute directly:
    ...
    # E[max(Z_i)²] = rho * E[W²] + (1-rho) * E[max(X_i)²] + 2*sqrt(rho)*sqrt(1-rho)*E[W]*E[max(X_i)]
    # = rho * 1 + (1-rho) * E[max(X_i)²] + 0
    e_mk2 = rho + (1.0 - rho) * e_max_iid_sq

    var_mk = e_mk2 - e_mk ** 2
    var_mk = max(var_mk, 0.0)
...
def probabilistic_sharpe_ratio(
    ...
    *,
    ...
    where σ(SR̂) = √(Var(SR̂)) from sharpe_ratio_variance().
...
def minimum_track_record_length(
    ...
    *,
    ...
    MinTRL = [ (z_α · σ_unit) / (SR - SR₀) ]²

    where σ_unit² = 1 - γ₃·SR + (γ₄-1)/4·SR² is the per-observation
    variance factor, and z_α = Φ⁻¹(1-α).
...
def critical_sharpe_ratio(
    ...
    *,
    ...
    as the benchmark instead of SR₀.
...
def sharpe_ratio_power(
    ...
    *,
    ...
    # Variance under the alternative (SR = SR₁)
...
def observed_fdr(
    ...
    *,
    ...
    Returns:
        oFDR in [0, 1].
    """
    # Implied alpha: P(SR̂ >= observed | H₀: true SR = SR₀)
    ...
    # Implied beta: P(SR̂ < observed | H₁: true SR = SR₁)
...
def expected_maximum_sharpe_ratio(
    ...
    where Z_i are standard normal and V is the variance of a single SR̂.

    This is a strict improvement over the simple √(2·ln(K)) approximation
    (which is the asymptotic limit for large K and ignores the SR₀ offset
